...
<script type="text/javascript">
	async function pNum(pid, p, no) {
		const res = await ajaxupdatecart(pid, 1)
		if(res.status == 200){
			location.href = "cartservlet?method=getCart";
		}
	}
	async function mNum(pid, p, no) {
		var num = -1; //数量
		...
		const res = await ajaxupdatecart(pid, num)
		if (res.status == 200) {
			location.href = "cartservlet?method=getCart";
		}
	}
	async function deleteCart(pid) {
		if (confirm("确认要删除吗?")) {
			const res = await ajaxupdatecart(pid, 0)
			if(res.status == 200){
				location.href = "cartservlet?method=getCart"
			}
		}
